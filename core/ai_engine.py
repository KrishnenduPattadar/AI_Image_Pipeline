...
from mediapipe.tasks import python
from mediapipe.tasks.python import vision
from deepface import DeepFace # changed the model for sex and age estimation
# --- PATHS ---
BASE_DIR = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
...
def analyze_human_pose(image_path):
    ...
    # Get confidence from the first landmark (Nose)
    confidence_score = detection_result.pose_landmarks[0][0].visibility
...
# --- 6. COLORS ---
def get_dominant_colors(image_path, mask_path, k=3):
    img = cv2.imread(image_path)
...
def estimate_age_sex(image_path, face_bbox_px):
    """
    Uses DeepFace to analyze age and gender.
    Expects a cropped face via face_bbox_px.
    """
    import cv2
    
    img = cv2.imread(image_path)
    if img is None: return "N/A", "N/A", 0.0

    h_img, w_img, _ = img.shape
    x, y, w, h = face_bbox_px 
    
    # 1. Apply padding & Force Integers
    padding = 20 # DeepFace likes a bit more context/padding
    ...
    # 2. Crop the face
    face_img = img[y1:y2, x1:x2]
    # 🔴 DEBUG: Save the crop to verify it!
    debug_path = os.path.join(os.path.dirname(image_path), "debug_face_crop.jpg")
    cv2.imwrite(debug_path, face_img)
    print(f"DEBUG: Saved crop to {debug_path}")

    if face_img.size == 0: return "N/A", "N/A", 0.0

    try:
        # 3. Run DeepFace Analysis
        # actions=['age', 'gender'] tells it what to look for.
        # enforce_detection=False because we already cropped the face.
        results = DeepFace.analyze(
            img_path=face_img, 
            actions=['age', 'gender'], 
            enforce_detection=True,
            detector_backend='opencv', # Fast backend since we already cropped
            silent=True
        )
        ...
        # DeepFace returns gender probabilities directly
        gender_data = result['gender'] # Returns {'Woman': 0.1, 'Man': 99.9}
        
        # Find dominant gender
        # Note: DeepFace output structure varies slightly by version, 
        # but usually returns dominant_gender or probabilities.
        
        if result['dominant_gender'] == 'Man':
            gender = "Male"
            confidence = result['gender']['Man'] / 100.0
        else:
            gender = "Female"
            confidence = result['gender']['Woman'] / 100.0

        return str(age), gender, float(confidence)

    except Exception as e:
        # If DeepFace fails to analyze the crop
